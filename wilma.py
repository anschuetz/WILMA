...
#                                                                   #
# jesko.anschuetz@linuxmuster.net - Oktober 2018                    #
#                                                                   #
#####################################################################
import pytz
import requests
...
debugflag = False
iframeFlag = False      # Error.log unten auf der Seite einbinden...
# debugflag = True
...
try:
        ...
        print("Fehler in der Konfigurationsdatei {} "
              "beim Schlüssel {}".format(configfile, detail))
        exit(1)
...
def fileExists(filename):
    try:
        ...
        print("unbehandelter Fehler in Funktion 'fileExists': >>>",
              detail, "<<<")
        exit(1)
...
def getUnixTimestampFromFile(filename):
        try:
            ...
            print("unbehandelter Fehler in Funktion "
                  "'getUnixTimestampFromFile': >>>", detail, "<<<")
            exit(1)
...
def getDatumUhrzeitFromUnixTimestamp(timestamp):
    if timestamp == 0:
        ...
        datumsformat = '%d.%m.%Y um %H:%M:%S'
        return datetime.fromtimestamp(timestamp).strftime(datumsformat)
...
htmlkopf += '   <title>WILMA - Wichtige Informationen Leserlich am Monitor '
htmlkopf += 'Angezeigt</title>\n'
htmlkopf += ' </head>\n'
htmlkopf += ' <body>\n'
...
htmlbody += '    </span>\n'
htmlbody += '    <div class="zeit">Seite erzeugt am '
htmlbody += now.strftime('%d.%m.%Y um %H:%M')
htmlbody += ' Das darf nicht länger als 2 Minuten her sein!</div>\n'

# Den Abschluss der Seite

if iframeFlag:
    htmlfuss += '   <div class="eintrag"><div class="beschreibung">\n'
    htmlfuss += '<iframe src="fehler.html" style="border:0px #FFFFFF none;" '
    htmlfuss += 'name="errorlog" scrolling="no" frameborder="0" '
    htmlfuss += 'align=aus marginheight="0px" marginwidth="0px" height="30" '
    htmlfuss += 'width="640"></iframe>'
    htmlfuss += '   </div></div>'
            ...
            for t in c.todos:
                if repr(t.completed) == "None":
                    ...
                    debugprint("completed Task {}"
                               " ... skipping".format(repr(t)))
                    continue
                # falls keine Fälligkeitszeit angegeben ist, anzeigen,
                # ansonsten prüfen ob noch aktuell
                if repr(t.due) == "None":
                    vor_ende = True
                else:
                    vor_ende = (t.due > now)
                # falls keine Startzeit angegeben ist, anzeigen,
                # ansonsten prüfen ob schon aktuell
                if repr(t.begin) == "None":
                    nach_beginn = True
                else:
                    nach_beginn = (now > t.begin)
                    # wenn noch nicht 100% UND Priorität #stimmt und
                    # zwischen den Daten, dann anzeigen.
                    # Nextcloud speichert die Priorität falschrum
                    # (1 = hoch und 9 = niedrig. 0 = ohne)
                ...
                if (prozent) & prioritaetstimmt & vor_ende & nach_beginn:
                    ...
                    debugprint("-"*50)
                    debugprint("-- {} -- Erzeuge Todo '{}', "
                               "{}% erledigt, Priorität-Task {}, "
                               "Priorität korrigiert: {} ".format(now,
                                                                  t.name,
                                                                  t.percent,
                                                                  t.priority,
                                                                  prio))
                    ...
                    if repr(t.description) != "None":
                            inhalt += '     <div class="beschreibung">'
                            inhalt += t.description.replace('\n', '<br />')
                            inhalt += '</div>\n'
                    inhalt += '   </div>\n'
        prio -= 1
except Exception as detail:
    ...
    for e in c.events:
        ...
        if show:
            ...
            if alter < 3600:
                ...
                div = "eintragalt"
            debugprint("-"*60)
            debugprint("-- {} -- Erzeuge Event '{}' Startzeit: {} Endzeit {}"
                       .format(now, e.name, e.begin, e.end))
            debugprint("Alter in Secunden: {}"
                       .format(str(abs(e.begin-now).seconds)))
...
entschuldigung = ""
# Modification-Time der Datei holen (Unix-Timestamp),
# in Datum/Zeit konvertieren und lesbar formatieren:
...
try:
    if excelDateiExistiert:
        with open_workbook(nameDerExceldatei, 'rb') as excelsheet:
            ...
            for zeile in zeilen:
                if (zeile[0] != "") & (zeile[1] != ""):
                    entschuldigung += '<tr><td><b>'
                    entschuldigung += '{}</td><td>'.format(zeile[0])
                    zeilenpuffer = ""
                    for spalte in zeile:
                        if spalte == "":
                            continue
            ...
            entschuldigungKopf += '<tr><th colspan="8">'
            entschuldigungKopf += ('Am {} sind insgesamt {} Schüler '
                                   'entschuldigt - zuletzt aktualisiert am {}'
                                   '</th>\n').format(datum,
                                                     anzahlESchueler,
                                                     lastModified)
    ...
    anzahlESchueler = 1000
    entschuldigungFuss = '</table></div>\n'

# Seite zusammensetzen.

if anzahlESchueler > 0:
    entschuldigungsblock = (entschuldigungKopf
                            + entschuldigung
                            + entschuldigungFuss)
else:
    entschuldigungsblock = '<div class="eintragtodo2"><div class="titel">'
    entschuldigungsblock = 'Stand {} ist kein Schüler '.format(lastModified)
    entschuldigungsblock = 'entschuldigt!</div></div>\n'
if excelDateiExistiert:
    pass
else:
    entschuldigungsblock = '<div class="eintragtodo9">'
    entschuldigungsblock = '<div class="titel">'
    entschuldigungsblock = 'Entschuldigungsdatei muss an die richtige Stelle'
    entschuldigungsblock = ' kopiert werden!!!</div></div>\n'
